...
variables, followed by a code-owned `gc.kind=review-quorum-finalize` control
step.
...
| `internal/controlkind` | Built-in `gc.kind` control catalog for graph-contract requirements, control routing classification, runtime requirements, graph route modes, and scope-check exemptions |
| `internal/bootstrap/packs/core/formulas/mol-review-quorum.toml` | Core two-lane review quorum formula scaffold |
| `internal/bootstrap/packs/core/formulas/mol-review-quorum-dynamic.toml` | Core dynamic review quorum workflow |
