...
func processReviewQuorumPlan(store beads.Store, bead beads.Bead, _ ProcessOptions) (ControlResult, error) {
	raw := strings.TrimSpace(bead.Metadata["gc.review_quorum_lanes_json"])
	if raw == "" {
	...
	closeMetadata := map[string]string{
		"gc.outcome":            "pass",
		"gc.output_json":        string(output),
		"gc.output_json_schema": reviewQuorumLanesSchema,
	}
...
func processReviewQuorumFinalize(store beads.Store, bead beads.Bead, _ ProcessOptions) (ControlResult, error) {
	subject := strings.TrimSpace(bead.Metadata["gc.review_quorum_subject"])
	baseRef := strings.TrimSpace(bead.Metadata["gc.review_quorum_base_ref"])
	...
	if sourceRef := strings.TrimSpace(bead.Metadata["gc.review_quorum_lanes_source"]); sourceRef != "" {
	...
	closeMetadata := map[string]string{
		"gc.outcome":               "pass",
		"gc.output_json":           string(rawSummary),
		"gc.output_json_schema":    reviewQuorumSummarySchema,
		"gc.review_quorum_verdict": summary.Verdict,
	}
	clearControllerSpawnErrorMetadata(closeMetadata)
	if err := updateMetadataAndClose(store, bead.ID, closeMetadata); err != nil {
...
func expectedReviewQuorumLaneIDs(all []beads.Bead, rootID, sourceRef string) ([]string, error) {
	...
	raw := strings.TrimSpace(source.Metadata["gc.output_json"])
	if raw == "" {
...
func collectReviewQuorumLaneBeads(all []beads.Bead, expected []string) ([]beads.Bead, error) {
	...
	for _, bead := range all {
		...
		laneID := strings.TrimSpace(bead.Metadata["gc.review_quorum_lane"])
...
func isReviewQuorumLaneBead(bead beads.Bead) bool {
	if strings.TrimSpace(bead.Metadata["gc.review_quorum_lane"]) == "" {
		return false
	}
	if strings.TrimSpace(bead.Metadata["gc.attempt"]) != "" {
		return false
	}
	return strings.TrimSpace(bead.Metadata["gc.output_json_schema"]) == reviewQuorumLaneSchema ||
		strings.TrimSpace(bead.Metadata["gc.kind"]) == "retry"
}

func sortReviewQuorumLaneBeads(lanes []beads.Bead) {
	sort.SliceStable(lanes, func(i, j int) bool {
		return strings.TrimSpace(lanes[i].Metadata["gc.review_quorum_lane"]) <
			strings.TrimSpace(lanes[j].Metadata["gc.review_quorum_lane"])
	})
}
...
func ensureExpectedReviewQuorumLanes(laneBeads []beads.Bead, expected []string) error {
	...
	for _, bead := range laneBeads {
		present[strings.TrimSpace(bead.Metadata["gc.review_quorum_lane"])] = struct{}{}
	}
	var missing []string
	for _, id := range expected {
...
func reviewQuorumLaneOutputFromBead(bead beads.Bead) reviewquorum.LaneOutput {
	raw := strings.TrimSpace(bead.Metadata["gc.output_json"])
	if raw != "" {
	...
	class := strings.TrimSpace(bead.Metadata["gc.failure_class"])
	reason := strings.TrimSpace(bead.Metadata["gc.failure_reason"])
	if class == "" {
...
func syntheticReviewQuorumLaneOutput(bead beads.Bead, failureClass, failureReason string) reviewquorum.LaneOutput {
	return reviewquorum.LaneOutput{
		LaneID:        strings.TrimSpace(bead.Metadata["gc.review_quorum_lane"]),
		Provider:      strings.TrimSpace(bead.Metadata["gc.provider"]),
		Model:         strings.TrimSpace(bead.Metadata["gc.model"]),
		Verdict:       reviewquorum.VerdictBlocked,
		Summary:       strings.TrimSpace(bead.Metadata["gc.final_disposition"]),
		FindingsCount: 0,
