...
func historySnapshotTurns(snapshot *worker.HistorySnapshot) ([]outputTurn, []string) {
	...
	for _, entry := range snapshot.Entries {
		...
		if len(turns) > 0 && outputTurnsEquivalent(turns[len(turns)-1], turn) {
			turns[len(turns)-1] = mergeOutputTurnMetadata(turns[len(turns)-1], turn)
			ids[len(ids)-1] = entry.ID
			continue
		}
...
func mergeOutputTurnMetadata(existing, incoming outputTurn) outputTurn {
