...
func TestEntryToTurnAddsImageAssetsFromTextAndToolResults(t *testing.T) {
	...
	message := transcriptMessage(t, []worker.TranscriptContentBlock{
		...
		{Type: "thinking", Thinking: "checked tool output"},
	})
	...
	if strings.Contains(turn.Text, "checked tool output") {
		t.Fatalf("turn.Text included thinking text inline: %q", turn.Text)
	}
	if len(turn.Trace) != 1 || turn.Trace[0].Kind != "thinking" || turn.Trace[0].Text != "checked tool output" {
		t.Fatalf("Trace = %#v, want thinking trace", turn.Trace)
	}
	if len(turn.Parts) != 6 {
...
func TestEntryToTurnCarriesThinkingBlocksInTrace(t *testing.T) {
	message := transcriptMessage(t, []worker.TranscriptContentBlock{
		{Type: "thinking", Thinking: "thinking through tool output"},
		{Type: "text", Text: "visible answer"},
	})
	turn := entryToTurn(&worker.TranscriptEntry{Type: "assistant", Message: message})

	if turn.Text != "visible answer" {
		t.Fatalf("Text = %q, want visible answer only", turn.Text)
	}
	if len(turn.Trace) != 1 || turn.Trace[0].Kind != "thinking" || turn.Trace[0].Text != "thinking through tool output" {
		t.Fatalf("Trace = %#v, want thinking trace", turn.Trace)
	}
	if got := mustMarshalString(t, turn); !strings.Contains(got, "thinking through tool output") {
		t.Fatalf("turn missing thinking trace: %s", got)
	}
}
...
func TestAppendOutputTurnDistinctDeduplicatesConsecutiveTraceOnlyTurns(t *testing.T) {
	...
	if turns[0].Timestamp != "2026-05-28T00:00:00Z" {
		t.Fatalf("deduped timestamp = %q, want latest duplicate timestamp", turns[0].Timestamp)
	}
}

func TestTranscriptOutputTurnsDeduplicatesToLatestEntry(t *testing.T) {
	first := time.Date(2026, 5, 30, 4, 38, 0, 0, time.UTC)
	second := time.Date(2026, 5, 30, 20, 40, 0, 0, time.UTC)
	items := transcriptOutputTurns([]*sessionlog.Entry{
		{
			UUID:      "codex-old",
			Type:      "user",
			Timestamp: first,
			Message:   sessionlogTextMessage(t, "user", "same reminder"),
		},
		{
			UUID:      "codex-new",
			Type:      "user",
	...
	if len(items) != 1 {
		t.Fatalf("items = %#v, want duplicate reminder collapsed", items)
	}
	if items[0].entryID != "codex-new" {
		t.Fatalf("entryID = %q, want latest duplicate entry ID", items[0].entryID)
	}
	if items[0].turn.Timestamp != "2026-05-30T20:40:00Z" {
		t.Fatalf("timestamp = %q, want latest duplicate timestamp", items[0].turn.Timestamp)
	}
}
...
func sessionlogTextMessage(t *testing.T, role, text string) json.RawMessage {
	t.Helper()
	content, err := json.Marshal(text)
	if err != nil {
		t.Fatalf("marshal text content: %v", err)
	}
	payload, err := json.Marshal(sessionlog.MessageContent{
		Role:    role,
		Content: content,
	})
	if err != nil {
		t.Fatalf("marshal sessionlog message: %v", err)
	}
	return payload
}
