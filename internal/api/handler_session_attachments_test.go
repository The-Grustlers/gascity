...
func TestHandleSessionAttachmentUploadRejectsMissingCSRF(t *testing.T) {
	fs := newSessionFakeState(t)
	h := newTestCityHandler(t, fs)
	info := createTestSession(t, fs.cityBeadStore, fs.sp, "Upload CSRF")

	body, contentType := multipartBody(t, "screen.png", "image/png", append([]byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n'}, bytes.Repeat([]byte{0}, 64)...))
	req := httptest.NewRequest(http.MethodPost, cityURL(fs, "/session/")+info.ID+"/attachments", body)
	req.Header.Set("Content-Type", contentType)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)

	if rec.Code != http.StatusForbidden {
		t.Fatalf("upload without CSRF status = %d, want %d; body: %s", rec.Code, http.StatusForbidden, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "csrf") {
		t.Fatalf("upload without CSRF body missing csrf detail: %s", rec.Body.String())
	}
}
...
func TestHandleSessionAttachmentDeleteRejectsMissingCSRF(t *testing.T) {
	fs := newSessionFakeState(t)
	h := newTestCityHandler(t, fs)
	info := createTestSession(t, fs.cityBeadStore, fs.sp, "Delete CSRF")
	resp := uploadTestSessionAttachment(t, h, fs, info.ID)
	...
	if rec.Code != http.StatusForbidden {
		t.Fatalf("delete without CSRF status = %d, want %d; body: %s", rec.Code, http.StatusForbidden, rec.Body.String())
	}
	if _, err := os.Stat(filepath.Dir(resp.Path)); err != nil {
		t.Fatalf("attachment dir should remain after rejected delete: %v", err)
	}
	if !strings.Contains(rec.Body.String(), "csrf") {
		t.Fatalf("delete without CSRF body missing csrf detail: %s", rec.Body.String())
	}
}
