...
describe("session cockpit", () => {
        ...
        return {
          data: {
            turns: [{
              role: "output",
              text: [
                "custom-router-cli: codex via gpt-5.5",
                "",
                "› [demo-city] director • 2026-05-23T18:55:26",
                "",
                ...
                "",
                "  gpt-5.5 high · /workspace/demo-city/.gc/agents/direct…",
              ].join("\n"),
  ...
  it("folds streamed context-only events into the next assistant output", async () => {
    ...
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      if (path === "/v0/city/{cityName}/sessions") {
        return { data: { items: [crewSession("s-ops")] } } as never;
      }
      if (path === "/v0/city/{cityName}/session/{id}") {
        return { data: { ...crewSession("s-ops"), provider: "codex" } } as never;
      }
      if (path === "/v0/city/{cityName}/session/{id}/pending") {
  ...
  it("treats autonomous control prompts as context instead of user chat", async () => {
    const controlPrompt = "Check mail, then run `gc hook worker` or `gc hook` for routed worker work. If work exists, claim and handle one item. If no other agent has worked recently and there is no routed infra work, drain-ack and stay idle unless the operator explicitly asks for monitoring, triage, or city work.";
    setupCrewCockpitDom();
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      if (path === "/v0/city/{cityName}/sessions") {
        return { data: { items: [crewSession("s-ops")] } } as never;
    ...
    expect(userMessages[0]?.textContent).not.toContain("gc hook worker");
    ...
    expect(details.textContent?.match(/gc hook worker/g)).toHaveLength(1);
  });

  it("renders deferred system reminders as compact reminder entries", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
      "- [session] Run your routed session review now. Use gc hook worker to inspect routed worker work.",
      "",
        ...
        return {
          data: {
            turns: [{
              role: "user",
              text: "what changed?",
              timestamp: "2026-05-29T23:36:00Z",
            }, {
              role: "user",
              text: reminder,
              timestamp: "2026-05-29T23:36:10Z",
            }, {
              role: "assistant",
              text: "No routed worker work.",
              timestamp: "2026-05-29T23:37:00Z",
            }],
            pagination: { has_older_messages: false, returned_message_count: 3, total_compactions: 0, total_message_count: 3 },
          },
        } as never;
      }
      throw new Error(`unexpected GET ${path}`);
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      expect(document.querySelector(".log-msg-reminder")).not.toBeNull();
    });

    const userMessages = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-user"));
    expect(userMessages).toHaveLength(1);
    expect(userMessages[0]?.textContent).toContain("what changed?");
    const reminderNode = document.querySelector<HTMLElement>(".log-msg-reminder")!;
    expect(reminderNode.classList.contains("log-msg-compact")).toBe(true);
    expect(reminderNode.querySelector("summary")?.textContent).toContain("Deferred session reminder");
    expect(reminderNode.textContent).toContain("Run your routed session review now");
    expect(reminderNode.textContent).not.toContain("<system-reminder>");
    expect(document.querySelector(".log-msg-assistant")?.textContent).toContain("No routed worker work.");
  });

  it("keeps reminder work cycles in chronological order", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
        ...
        return {
          data: {
            turns: [{
              ...
              role: "assistant",
              text: "First patrol preflight.",
              timestamp: "2026-05-29T23:36:01Z",
            }, {
              role: "assistant",
              parts: [{ id: "call-1", type: "tool", tool: "exec_command", input: { cmd: "gc hook worker" } }],
              timestamp: "2026-05-29T23:36:02Z",
            }, {
              role: "assistant",
              text: "No routed worker work.",
              timestamp: "2026-05-29T23:36:03Z",
            }, {
              role: "user",
              text: reminder,
              ...
              role: "assistant",
              text: "Second patrol preflight.",
              timestamp: "2026-05-30T00:06:01Z",
            }, {
              role: "assistant",
              parts: [{ id: "call-2", type: "tool", tool: "exec_command", input: { cmd: "gc hook worker" } }],
              timestamp: "2026-05-30T00:06:02Z",
            }, {
              role: "assistant",
              text: "Still idle.",
              timestamp: "2026-05-30T00:06:03Z",
            }],
            pagination: { has_older_messages: false, returned_message_count: 8, total_compactions: 0, total_message_count: 8 },
          },
        } as never;
      }
      throw new Error(`unexpected GET ${path}`);
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      expect(document.querySelectorAll(".log-msg-assistant")).toHaveLength(2);
    });

    expect(document.querySelectorAll(".log-msg-reminder")).toHaveLength(2);
    const messages = Array.from(document.querySelectorAll<HTMLElement>(".log-msg"));
    expect(messages.map((node) => {
      if (node.classList.contains("log-msg-reminder")) return "reminder";
      return node.querySelector(".log-msg-body")?.textContent?.trim() ?? "";
    })).toEqual([
      "reminder",
      "No routed worker work.",
      "reminder",
      "Still idle.",
    ]);

    const assistantMessages = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant"));
    const firstActivity = assistantMessages[0]?.querySelector<HTMLElement>(".log-msg-activity");
    const secondActivity = assistantMessages[1]?.querySelector<HTMLElement>(".log-msg-activity");
    expect(firstActivity?.textContent).toContain("First patrol preflight.");
    expect(firstActivity?.textContent).toContain("Tool running · exec_command");
    expect(secondActivity?.textContent).toContain("Second patrol preflight.");
    expect(secondActivity?.textContent).toContain("Tool running · exec_command");
  });

  it("does not attach stale pre-reminder activity to a later reply after refresh", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
        ...
        return {
          data: {
            turns: [{
              ...
              role: "user",
              text: reminder,
              timestamp: "2026-05-31T08:18:00Z",
            }, {
              role: "assistant",
              parts: [{ id: "old-call", type: "tool", tool: "exec_command", input: { cmd: "gc hook stale" } }],
              timestamp: "2026-05-31T07:00:00Z",
            }, {
              role: "assistant",
              parts: [{ id: "current-call", type: "tool", tool: "exec_command", input: { cmd: "gc hook worker" } }],
              timestamp: "2026-05-31T09:37:00Z",
            }, {
              role: "assistant",
              text: "No routed worker work from `gc hook worker`.\n\nStaying idle.",
              timestamp: "2026-05-31T09:38:00Z",
            }],
            pagination: { has_older_messages: false, returned_message_count: 5, total_compactions: 0, total_message_count: 5 },
          },
        } as never;
      }
      throw new Error(`unexpected GET ${path}`);
    ...
    expect(document.querySelectorAll(".log-msg-reminder")).toHaveLength(2);
    const standalone = document.querySelector<HTMLElement>(".log-msg-activity-standalone")!;
    expect(standalone?.textContent).toContain("gc hook stale");

    const assistantActivity = document.querySelector<HTMLElement>(".log-msg-assistant .log-msg-activity")!;
    expect(assistantActivity?.querySelector(".log-msg-activity-summary")?.textContent).toContain("1 tool");
    expect(assistantActivity?.querySelector(".log-msg-activity-summary")?.textContent).not.toContain("2 tools");
    expect(assistantActivity?.textContent).toContain("gc hook worker");
    expect(assistantActivity?.textContent).not.toContain("gc hook stale");
  });

  it("shows when a session completes a reminder turn without visible output", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
      "- [session] Run your routed session review now.",
      "",
      "Handle them after this turn.",
      "</system-reminder>",
    ].join("\n");
        ...
        return {
          data: {
            turns: [{
              ...
              role: "system",
              text: "Session completed this turn without a visible response.",
    ...
    expect(systemNode.textContent).toContain("Session completed this turn without a visible response.");
  });

  it("keeps repeated silent reminder completions visible", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
    ...
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      ...
      if (path === "/v0/city/{cityName}/session/{id}/transcript") {
        return {
          data: {
            turns: Array.from({ length: 4 }).flatMap((_, index) => [{
              role: "system",
              text: `<environment_context>\n  <cwd>/tmp/city-${index}</cwd>\n</environment_context>`,
              timestamp: `2026-05-30T12:0${index}:00Z`,
            }, {
              role: "user",
              text: reminder,
              timestamp: `2026-05-30T12:0${index}:01Z`,
            }, {
              role: "system",
              text: "Session completed this turn without a visible response.",
              timestamp: `2026-05-30T12:0${index}:30Z`,
            }]),
            pagination: { has_older_messages: false, returned_message_count: 12, total_compactions: 0, total_message_count: 12 },
          },
        } as never;
      }
      throw new Error(`unexpected GET ${path}`);
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      expect(document.querySelectorAll(".log-msg-reminder")).toHaveLength(4);
    });

    expect(document.body.textContent).not.toContain("reminder attempts completed without visible output");
    const noVisibleResponseNodes = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-system"))
      .filter((node) => node.textContent?.includes("Session completed this turn without a visible response."));
    expect(noVisibleResponseNodes).toHaveLength(4);
  });

  it("keeps consecutive repeated reminders visible before the next visible reply", async () => {
          ...
          data: {
            turns: [
              ...Array.from({ length: 6 }).map((_, index) => ({
                role: "user",
                text: reminderFor(index + 1),
                timestamp: `2026-05-31T10:${String(index).padStart(2, "0")}:00Z`,
              })),
              {
                role: "assistant",
                text: "No routed worker work from `gc hook worker`.\n\nStaying idle.",
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      expect(document.querySelectorAll(".log-msg-reminder")).toHaveLength(6);
    });

    expect(document.body.textContent).not.toContain("repeated reminders collapsed");
    expect(document.querySelector(".log-msg-assistant")?.textContent).toContain("No routed worker work");
  });

  it("keeps repeated idle reminder responses visible", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
        ...
        return {
          data: {
            turns: Array.from({ length: 4 }).flatMap((_, index) => [{
              role: "user",
              text: reminder,
              timestamp: `2026-05-30T13:0${index}:00Z`,
            }, {
              role: "assistant",
              text: "Hook gate.",
              timestamp: `2026-05-30T13:0${index}:01Z`,
            }, {
              role: "assistant",
              parts: [{ id: `call-${index}`, type: "tool", tool: "exec_command", input: { cmd: "gc hook worker" } }],
              timestamp: `2026-05-30T13:0${index}:02Z`,
            }, {
              role: "assistant",
              text: "No routed worker work from `gc hook worker`.\n\nStaying idle.",
              timestamp: `2026-05-30T13:0${index}:03Z`,
            }]),
            pagination: { has_older_messages: false, returned_message_count: 16, total_compactions: 0, total_message_count: 16 },
          },
        } as never;
      }
      throw new Error(`unexpected GET ${path}`);
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      expect(document.querySelectorAll(".log-msg-assistant")).toHaveLength(4);
    });

    expect(document.body.textContent).not.toContain("routine reminder checks collapsed");
    expect(document.body.textContent).not.toContain("idle reminder checks collapsed");
    expect(document.querySelectorAll(".log-msg-reminder")).toHaveLength(4);
    const assistants = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant"));
    expect(assistants.filter((node) => node.querySelector(".log-msg-body")?.textContent?.includes("No routed worker work"))).toHaveLength(4);
    expect(assistants.every((node) => node.querySelector(".log-msg-activity")?.textContent?.includes("Hook gate."))).toBe(true);
    expect(assistants.every((node) => node.querySelector(".log-msg-activity")?.textContent?.includes("Tool running · exec_command"))).toBe(true);
  });
  ...
  it("renders partial system reminder bodies as compact reminder entries", async () => {
    ...
    expect(document.querySelectorAll(".log-msg-user")).toHaveLength(0);
    const reminderNode = document.querySelector<HTMLElement>(".log-msg-reminder")!;
    expect(reminderNode.classList.contains("log-msg-compact")).toBe(true);
    expect(reminderNode.querySelector("summary")?.textContent).toContain("Deferred session reminder");
    expect(reminderNode.textContent).toContain("Run your routed session review now");
  ...
  it("does not fold streamed assistant progress across a reminder boundary", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
      "- [session] Run the next scheduled check.",
      "",
      "Handle them after this turn.",
      "</system-reminder>",
    ].join("\n");
    ...
    streamCallback?.({
      type: "turn",
      data: { turns: [{ role: "assistant", text: "Checking first scheduled item.", timestamp: "2026-05-31T20:00:01Z" }] },
    });
    streamCallback?.({
      type: "turn",
      data: { turns: [{ role: "user", text: reminder, timestamp: "2026-05-31T20:15:00Z" }] },
    });
    streamCallback?.({
      type: "turn",
      data: { turns: [{ role: "assistant", text: "Checking second scheduled item.", timestamp: "2026-05-31T20:15:01Z" }] },
    });
    ...
    expect(messages.map((node) => {
      ...
      "Checking first scheduled item.",
      "reminder",
      "Checking second scheduled item.",
    ]);
    expect(document.querySelectorAll(".log-msg-assistant")).toHaveLength(2);
    expect(document.querySelectorAll(".log-msg-assistant .log-msg-activity")).toHaveLength(0);
  });
  ...
  it("submits chat messages through the session submit endpoint", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({
      ...
      mime_type: "image/png",
      name: "screenshot.png",
      path: "/tmp/test-city/.gc/dashboard/attachments/s-ops/att-123/screenshot.png",
      size: 400_000,
      url: "/v0/city/mc-city/session/s-ops/attachments/att-123/screenshot.png",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/v0/city/mc-city/session/s-ops/attachments",
    ...
    expect(posts[0]?.body?.message).toContain("![screenshot.png](/v0/city/mc-city/session/s-ops/attachments/att-123/screenshot.png)");
    expect(posts[0]?.body?.message).toContain("Local file: /tmp/test-city/.gc/dashboard/attachments/s-ops/att-123/screenshot.png");
    ...
    expect(document.querySelector<HTMLImageElement>(".log-msg-image")?.getAttribute("src")).toBe("/v0/city/mc-city/session/s-ops/attachments/att-123/screenshot.png");
    expect(document.getElementById("log-drawer-status")?.textContent).toBe("Sent · waiting for response");
    expect(document.querySelector(".log-msg-local-response-pending")?.textContent).toContain("Waiting for session response...");
    expect(document.getElementById("log-drawer-count")?.textContent).toBe("2 entries");
    ...
    streamCallback?.({
      ...
      data: {
        turns: [{
          role: "assistant",
          text: "Queue looks clear.",
          timestamp: "2026-04-18T20:01:03Z",
        }],
      },
    });

    expect(document.querySelector(".log-msg-local-response-pending")).toBeNull();
    expect(document.getElementById("log-drawer-status")?.textContent).toBe("");
    expect(document.getElementById("log-drawer-messages")?.textContent).toContain("Queue looks clear.");
    expect(document.getElementById("log-drawer-count")?.textContent).toBe("2 entries");
  });
  ...
  it("deletes draft image attachments when pending chips are removed", async () => {
    ...
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      ...
      if (path === "/v0/city/{cityName}/session/{id}") {
        return { data: crewSession("s-ops") } as never;
      }
      if (path === "/v0/city/{cityName}/session/{id}/pending") {
        return { data: { pending: false } } as never;
      }
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({
        ...
        mime_type: "image/png",
        name: "remove-me.png",
        path: "/tmp/test-city/.gc/dashboard/attachments/s-ops/att-delete/remove-me.png",
        size: 12,
        status: "draft",
        url: "/v0/city/mc-city/session/s-ops/attachments/att-delete/remove-me.png",
      }), { headers: { "Content-Type": "application/json" }, status: 201 });
    });
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        "/v0/city/mc-city/session/s-ops/attachments/att-delete",
  ...
  it("renders transcript image assets through attachment and session asset URLs", async () => {
          ...
          data: {
            turns: [{
              ...
              text: [
                ...
                "Attached images:",
                "1. sent.png",
                "   ![sent](/v0/city/mc-city/session/s-ops/attachments/att-3/sent.png)",
                "   Local file: /tmp/test-city/.gc/dashboard/attachments/s-ops/att-3/sent.png",
              ...
              text: [
                "Viewed Image",
                "└ ../../dashboard/attachments/s-linked/86e29c944f316507c41b1983ec6bed5c/remote-",
                "play-motion-proof.png",
              ].join("\n"),
              timestamp: "2026-04-18T19:59:45Z",
            }, {
              role: "assistant",
              text: "Saw second image: remote-play-motion-proof.png.",
              ...
              role: "assistant",
              text: [
                "Look at ![existing](/v0/city/mc-city/session/s-ops/attachments/att-1/existing.png)",
                "Attached images:",
                "1. wrapped.png",
                "   ![wrapped](/v0/city/mc-city/session/s-ops/",
                "attachments/att-2/wrapped.png)",
                "   Local file: /tmp/test-city/.gc/dashboard/attachments/s-ops/att-2/wrapped.png",
                "Use the local file path to inspect the image when needed. Do not inline or decode base64.",
    ...
    expect(srcs.filter((src) => src === "/v0/city/mc-city/session/s-ops/attachments/att-3/sent.png")).toHaveLength(1);
    expect(srcs).not.toContain("/v0/city/mc-city/session/s-ops/asset?path=sent.png");
    expect(srcs.filter((src) => src === "/v0/city/mc-city/session/s-linked/attachments/86e29c944f316507c41b1983ec6bed5c/remote-play-motion-proof.png")).toHaveLength(1);
    expect(srcs).not.toContain("/v0/city/mc-city/session/s-ops/asset?path=remote-play-motion-proof.png");
    expect(srcs).toContain("/v0/city/mc-city/session/s-ops/attachments/att-1/existing.png");
    expect(srcs).toContain("/v0/city/mc-city/session/s-ops/attachments/att-2/wrapped.png");
    expect(srcs).toContain("/v0/city/mc-city/session/s-ops/asset?path=shots%2Flocal.png");
    expect(srcs).toContain("/v0/city/mc-city/session/s-ops/asset?path=shots%2Fplain.webp");
    expect(document.getElementById("log-drawer-messages")?.textContent).toContain("large encoded image data omitted");
  ...
  it("collapses context and tool plumbing while keeping tool images visible once", async () => {
    ...
    const srcs = Array.from(document.querySelectorAll<HTMLImageElement>(".log-msg-image")).map((img) => img.getAttribute("src"));
    expect(srcs).toEqual(["/v0/city/mc-city/session/s-ops/asset?path=shots%2Fpreview.png"]);
  });
  ...
  it("keeps interim agent updates visible while folding tool details into adjacent replies", async () => {
    setupCrewCockpitDom();
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      if (path === "/v0/city/{cityName}/sessions") {
        return { data: { items: [crewSession("s-ops")] } } as never;
    ...
    document.querySelector<HTMLButtonElement>(".agent-log-link")?.click();
    await waitFor(() => {
      const bodies = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant .log-msg-body"));
      expect(bodies.some((node) => node.textContent?.includes("Saw second image"))).toBe(true);
    });

    const assistantMessages = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant"));
    expect(assistantMessages).toHaveLength(3);
    expect(assistantMessages[0]?.querySelector(".log-msg-body")?.textContent).toContain("Looking for image files");
    expect(assistantMessages[1]?.querySelector(".log-msg-body")?.textContent).toContain("Found another image");
    expect(assistantMessages[2]?.querySelector(".log-msg-body")?.textContent).toContain("Saw second image");

    const firstActivity = assistantMessages[0]?.querySelector<HTMLDetailsElement>(".log-msg-activity");
    expect(firstActivity?.querySelector("summary")?.textContent).toContain("reasoning");
    expect(firstActivity?.textContent).toContain("Use the existing attachments directory first.");

    const secondActivity = assistantMessages[1]?.querySelector<HTMLDetailsElement>(".log-msg-activity");
    expect(secondActivity?.querySelector("summary")?.textContent).toContain("1 tool");
    expect(secondActivity?.textContent).toContain("Tool · exec_command");
    expect(secondActivity?.textContent).toContain("attachments/preview.png");

    const activity = assistantMessages[2]?.querySelector<HTMLDetailsElement>(".log-msg-activity");
    expect(activity?.open).toBe(false);
    expect(activity?.querySelector("summary")?.textContent).toContain("Worked");
    expect(activity?.querySelector("summary")?.textContent).toContain("1 tool");
    ...
    const srcs = Array.from(document.querySelectorAll<HTMLImageElement>(".log-msg-image")).map((img) => img.getAttribute("src"));
    expect(srcs).toEqual(["/v0/city/mc-city/session/s-ops/asset?path=attachments%2Fpreview.png"]);
  });

  it("does not carry stale activity across user turn boundaries", async () => {
    setupCrewCockpitDom();
    vi.spyOn(api, "GET").mockImplementation(async (path: string) => {
      if (path === "/v0/city/{cityName}/sessions") {
        return { data: { items: [crewSession("s-ops")] } } as never;
      ...
      if (path === "/v0/city/{cityName}/session/{id}/transcript") {
        return {
          data: {
            turns: [{
              role: "assistant",
              text: "Old preflight from yesterday.",
              timestamp: "2026-05-29T19:00:00Z",
            }, {
              role: "assistant",
              parts: [{ id: "old-call", type: "tool", tool: "exec_command", input: { cmd: "gc old" } }],
              timestamp: "2026-05-29T19:00:01Z",
            }, {
              role: "user",
              text: "Run the current session check.",
              timestamp: "2026-05-30T21:57:49Z",
            }, {
              role: "assistant",
              text: "Checking current hook.",
              timestamp: "2026-05-30T21:57:52Z",
            }, {
              role: "assistant",
              parts: [{ id: "new-call", type: "tool", tool: "exec_command", input: { cmd: "gc hook worker" } }],
              timestamp: "2026-05-30T21:57:54Z",
            }, {
              role: "assistant",
              text: "No routed worker work.",
              timestamp: "2026-05-30T21:57:57Z",
            }],
            pagination: { has_older_messages: false, returned_message_count: 6, total_compactions: 0, total_message_count: 6 },
          },
        } as never;
    ...
    await waitFor(() => {
      const bodies = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant .log-msg-body"));
      expect(bodies.some((node) => node.textContent?.includes("No routed worker work."))).toBe(true);
    });

    const assistants = Array.from(document.querySelectorAll<HTMLElement>(".log-msg-assistant"));
    const current = assistants.find((node) => node.querySelector(".log-msg-body")?.textContent?.includes("No routed worker work."))!;
    const activity = current.querySelector<HTMLDetailsElement>(".log-msg-activity")!;
    const summary = activity.querySelector(".log-msg-activity-summary")?.textContent ?? "";
    expect(summary).toContain("Worked");
    expect(summary).toContain("1 tool");
    expect(summary).not.toContain("26h");
    expect(assistants.some((node) => node.querySelector(".log-msg-body")?.textContent?.includes("Checking current hook."))).toBe(false);
    expect(activity.textContent).toContain("Checking current hook.");
    expect(activity.textContent).toContain("gc hook worker");
    expect(activity.textContent).not.toContain("Old preflight from yesterday.");
    expect(activity.textContent).not.toContain("gc old");
  });
  ...
  it("posts pending interaction responses from the cockpit card", async () => {
    ...
    vi.spyOn(api, "POST").mockImplementation(async (path: string, options?: unknown) => {
      ...
      return { data: { id: "s-ops", status: "accepted" } } as never;
    });
