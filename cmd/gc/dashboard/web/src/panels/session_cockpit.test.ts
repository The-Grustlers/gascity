...
describe("session cockpit", () => {
  ...
  it("renders deferred system reminders as compact reminder entries", async () => {
    ...
    expect(reminderNode.textContent).toContain("Run your routed mayor autonomy/infra review now");
    expect(reminderNode.textContent).not.toContain("<system-reminder>");
    expect(document.querySelector(".log-msg-assistant")?.textContent).toContain("No routed mayor work.");
  });

  it("uses reminders as boundaries for collapsed autonomous work", async () => {
    const reminder = [
      "<system-reminder>",
      "You have a deferred reminder that was queued until a safe boundary:",
      "",
        ...
        return {
          data: {
            turns: [{
              ...
              role: "assistant",
              parts: [{ id: "call-2", type: "tool", tool: "exec_command", input: { cmd: "gc hook mayor" } }],
    ...
    expect(assistantMessages[0]?.querySelector(".log-msg-body")?.textContent).toContain("No routed mayor work.");
    expect(assistantMessages[1]?.querySelector(".log-msg-body")?.textContent).toContain("Still idle.");
    ...
    expect(firstActivity?.textContent).not.toContain("Second patrol preflight.");
