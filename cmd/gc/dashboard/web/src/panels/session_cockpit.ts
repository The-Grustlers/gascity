...
const TRANSCRIPT_INITIAL_TURN_LIMIT = 100;
const TRANSCRIPT_OLDER_TURN_LIMIT = 150;
  ...
  const res = await api.GET("/v0/city/{cityName}/session/{id}/transcript", {
    params: {
      ...
      query: {
        ...
        limit: prepend ? TRANSCRIPT_OLDER_TURN_LIMIT : TRANSCRIPT_INITIAL_TURN_LIMIT,
        tail: "0",
      },
    },
  });
...
function appendStreamTurns(turns: TranscriptTurn[]): void {
  ...
  acknowledgeLocalResponses(displayTurns);
...
async function submitLogDrawerMessage(): Promise<void> {
  ...
  statusEl?.replaceChildren(document.createTextNode("Sent · waiting for response"));
...
function appendLocalTurn(role: string, text: string, attachments: ChatAttachment[] = []): void {
  ...
  const node = renderTurn(role, text, timestamp, attachments);
  if ((role ?? "").toLowerCase() === "user") {
    const id = `local-${Date.now()}-${nextLocalSubmissionID++}`;
    const responseID = `local-response-${id}`;
    ...
    const responseNode = renderTurn("system", "Waiting for session response...", timestamp);
    responseNode.dataset.localResponseId = responseID;
    responseNode.classList.add("log-msg-local-response-pending");
    messagesEl.append(node, responseNode);
    logCount += 2;
    updateLogCount();
    scrollLogDrawerToBottom();
    return;
...
function acknowledgeLocalSubmissions(turns: DisplayTurn[]): void {
  ...
  for (const turn of turns) {
    ...
    const index = pendingLocalSubmissions.findIndex((submission) => !submission.acknowledged && localSubmissionMatchesTurn(submission, turn));
    if (index < 0) continue;
    const submission = pendingLocalSubmissions[index];
    byId("log-drawer-messages")?.querySelector(`[data-local-submit-id="${submission?.id}"]`)?.remove();
    logCount = Math.max(0, logCount - 1);
    if (submission?.responseID) {
      submission.acknowledged = true;
    } else {
      pendingLocalSubmissions.splice(index, 1);
    }
  }
}

function acknowledgeLocalResponses(turns: DisplayTurn[]): void {
  const responseTurns = turns.filter((turn) => !isUserMessageDisplayTurn(turn));
  if (responseTurns.length === 0 || pendingLocalSubmissions.length === 0) return;
  let remaining = responseTurns.length;
  for (const submission of pendingLocalSubmissions) {
    if (!submission.responseID) continue;
    const node = byId("log-drawer-messages")?.querySelector(`[data-local-response-id="${submission.responseID}"]`);
    if (node) {
      node.remove();
      logCount = Math.max(0, logCount - 1);
    }
    submission.responseID = undefined;
    remaining -= 1;
    if (remaining <= 0) break;
  }
  pendingLocalSubmissions = pendingLocalSubmissions.filter((submission) => !submission.acknowledged || submission.responseID);
  if (!pendingLocalSubmissions.some((submission) => submission.responseID)) {
...
function prunePendingLocalSubmissions(): void {
  ...
  pendingLocalSubmissions = pendingLocalSubmissions.filter((submission) => submission.createdAt >= cutoff || submission.responseID);
}
