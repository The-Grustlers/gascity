...
export function isTimelineBoundaryDisplayTurn(turn: DisplayTurn): boolean {
  return isUserMessageTurn(turn) || isReminderTurn(turn);
}
...
export function expandTranscriptTurns(turns: TranscriptTurn[]): DisplayTurn[] {
  ...
  return collapseOutputRunsByTimelineBoundaries(attachActivityToOutputTurns(dedupeConsecutiveDisplayTurns(expanded)));
}
...
function expandTranscriptTurn(role: string, text: string, timestamp: string | undefined, assets: TranscriptAsset[] = [], trace: TranscriptTrace[] = []): DisplayTurn[] {
  if (!isTerminalTranscript(role, text)) {
    return [normalizeDisplayTurn({ assets, role, text, timestamp, trace })];
  }
  ...
  if (parsed.length > 0) {
    ...
    return parsed.map(normalizeDisplayTurn);
  }
  return [normalizeDisplayTurn({ assets, role, text, timestamp, trace })];
}
...
function normalizeDisplayTurn(turn: DisplayTurn): DisplayTurn {
  if (!isSystemReminderText(turn.text ?? "")) return turn;
  return {
    ...turn,
    role: "reminder",
    text: systemReminderDisplayText(turn.text),
  };
}
...
function dedupeConsecutiveDisplayTurns(turns: DisplayTurn[]): DisplayTurn[] {
  ...
  let previousKey = "";
  for (const turn of turns) {
    if (isTimelineBoundaryDisplayTurn(turn)) {
      deduped.push(turn);
      previousKey = "";
      continue;
    }
...
function attachActivityToOutputTurns(turns: DisplayTurn[]): DisplayTurn[] {
  ...
  let lastTimelineBoundaryTime: number | null = null;
  const flush = () => {
  ...
  for (const turn of turns) {
    if (isTimelineBoundaryDisplayTurn(turn)) {
      flush();
      grouped.push(turn);
      lastTimelineBoundaryTime = timestampMillis(turn.timestamp);
      continue;
    }
    ...
    if (pendingItems.length > 0 && isOutputMessageTurn(turn)) {
      const partitioned = partitionPendingActivityByBoundary(pendingItems, lastTimelineBoundaryTime);
      if (partitioned.before.length > 0) {
        grouped.push({
          activity: partitioned.before,
          assets: pendingAssets,
          role: "tool",
          text: "",
          timestamp: partitioned.before[0]?.timestamp,
        });
      }
      const attachItems = partitioned.after;
      grouped.push({
        ...turn,
        activity: attachItems.length > 0 ? appendActivityItems(attachItems, turnActivity) : turnActivity,
        assets: appendTranscriptAssets(partitioned.before.length > 0 ? [] : pendingAssets, turn.assets ?? []),
        trace: attachItems.length > 0 || turnActivity.length > 0 ? [] : turn.trace,
      });
      pendingItems = [];
      pendingAssets = [];
      lastTimelineBoundaryTime = null;
      continue;
    }
    flush();
    grouped.push({
...
function collapseOutputRunsByTimelineBoundaries(turns: DisplayTurn[]): DisplayTurn[] {
  const collapsed: DisplayTurn[] = [];
  let run: DisplayTurn[] = [];

  const flushRun = () => {
    if (run.length > 0) {
      collapsed.push(...collapseOutputRun(run));
      run = [];
    }
  };
...
function collapseOutputRun(run: DisplayTurn[]): DisplayTurn[] {
  ...
  for (let index = 0; index < run.length; index += 1) {
    ...
    if (index < finalOutputIndex && isCollapsibleProgressOutput(turn)) {
    ...
    if (index === finalOutputIndex && isOutputMessageTurn(turn) && pendingItems.length > 0) {
      collapsed.push({
        ...turn,
...
function isCollapsibleProgressOutput(turn: DisplayTurn): boolean {
  if (!isOutputMessageTurn(turn)) return false;
  if ((turn.activity ?? []).length > 0) return false;
  const text = turn.text.trim();
  if (text === "" || isNoVisibleResponseText(text)) return false;
  const nonEmptyLines = text.split("\n").filter((line) => line.trim() !== "");
  return text.length <= 180 && nonEmptyLines.length <= 2;
}

function partitionPendingActivityByBoundary(items: ActivityItem[], boundaryTime: number | null): { before: ActivityItem[]; after: ActivityItem[] } {
  if (boundaryTime === null) return { before: [], after: items };
  const before: ActivityItem[] = [];
  const after: ActivityItem[] = [];
  for (const item of items) {
    const itemTime = timestampMillis(item.timestamp);
    if (itemTime !== null && itemTime < boundaryTime) {
      before.push(item);
    } else {
      after.push(item);
    }
...
function isReminderTurn(turn: DisplayTurn): boolean {
  const presentation = turnPresentation(turn.role, turn.text.trim(), turn.trace ?? []);
  return presentation.kind === "reminder";
}
...
function derivedActivityItemsForTurn(turn: DisplayTurn): ActivityItem[] {
  ...
  if (presentation.kind === "reminder") return items;
...
function roleForTerminalPrompt(prompt: string): string {
  ...
  if (isSystemReminderText(trimmed)) return "reminder";
  if (
    /^\[[^\]]+\]\s+\S+\s+•/.test(trimmed) ||
...
function turnPresentation(role: string, text: string, trace: TranscriptTrace[]): TurnPresentation {
  ...
  if (normalized === "reminder" || isSystemReminderText(text)) {
    const detailText = systemReminderDisplayText(text);
    return {
      detailText,
      kind: "reminder",
      label: "reminder",
      summary: systemReminderSummary(detailText),
    };
  }
...
function isSystemReminderText(text: string): boolean {
  const trimmed = text.trim();
  if (/^<system-reminder\b/i.test(trimmed) && /<\/system-reminder>\s*$/i.test(trimmed)) {
    return true;
  }
  return /<\/system-reminder>\s*$/i.test(trimmed) && looksLikeSystemReminderBody(trimmed);
}

function looksLikeSystemReminderBody(text: string): boolean {
  return (
    /^You have a deferred reminder that was queued until a safe boundary:/i.test(text) ||
    /^-\s*\[[^\]]+\]\s+/m.test(text) ||
    /^New message in shared conversation\s+/i.test(text)
  );
}

function systemReminderDisplayText(text: string): string {
  let body = text.trim();
  body = body.replace(/^<system-reminder\b[^>]*>\s*/i, "").replace(/\s*<\/system-reminder>\s*$/i, "").trim();
  body = body.replace(
    /^You have a deferred reminder that was queued until a safe boundary:\s*/i,
    "Deferred reminder queued until a safe boundary.",
  );
  body = body.replace(/\n\s*Handle them after this turn\.\s*$/i, "").trim();
  return body;
}

function systemReminderSummary(text: string): string {
  const detailText = systemReminderDisplayText(text);
  const match = detailText.match(/-\s+\[([^\]]+)\]\s+([^\n]+)/);
  if (match?.[1] && match[2]) {
    return `Deferred ${match[1].trim()} reminder · ${summarizeActivityText(match[2], 72)}`;
  }
  return summarizeActivityText(firstNonEmptyLine(detailText), 90) || "System reminder";
}
...
function isNoVisibleResponseText(text: string): boolean {
  const trimmed = text.trim();
  return trimmed === "Session completed this turn without a visible response."
    || trimmed === "Codex completed this turn without a visible response.";
}
...
function traceThinkingTexts(trace: TranscriptTrace[]): string[] {
  ...
  for (const item of trace) {
    if (!isReasoningTraceKind(item.kind)) continue;
...
function isReasoningTraceKind(kind: string | undefined): boolean {
  const normalized = (kind ?? "").toLowerCase();
  return normalized === "thinking" || normalized === "reasoning" || normalized === "reasoning_summary";
}
