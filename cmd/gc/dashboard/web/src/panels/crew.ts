...
function renderSessionsWorkspace(
  ...
  if (rows.length === 0) {
    ...
    renderSimpleEmpty(list, "No running sessions");
  ...
  rows.forEach((session) => {
    ...
    const row = el("div", {
      ...
      el("div", { class: "session-row-main" }, [
        ...
        sessionRowStateBadge(state),
      ]),
      el("div", { class: "session-row-meta" }, sessionRowMeta(session, activity)),
...
function sessionRowMeta(session: SessionRecord, activity: ReturnType<typeof calculateActivity>): (Node | string)[] {
  const kind = sessionKind(session);
  const location = sessionLocation(session);
  const labels = location === kind ? [kind] : [kind, location];
  return [
    ...labels.map((label) => el("span", {}, [label])),
...
function sessionRowStateBadge(state: string): HTMLElement | null {
  if (state === "active" || state === "idle") return null;
  return el("span", { class: `badge ${sessionStateBadgeClass(state)}` }, [state]);
}
